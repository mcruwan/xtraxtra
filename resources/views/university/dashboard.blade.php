<x-university-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <!-- Notification Alert for Unread Replies -->
            @if($ticketStats['unread_replies'] > 0)
                <div id="ticket-notification" class="mb-6 flex items-center p-4 text-blue-800 rounded-lg bg-blue-50 border border-blue-200" role="alert">
                    ...
                    <div class="ms-3 text-sm font-medium flex-1">
                        <span class="font-bold">{{ $ticketStats['unread_replies'] }}</span> support {{ Str::plural('ticket', $ticketStats['unread_replies']) }} {{ $ticketStats['unread_replies'] == 1 ? 'has' : 'have' }} new replies from our support team.
                        <a href="{{ route('university.tickets.index') }}" class="font-semibold underline hover:no-underline ms-2">View tickets →</a>
                    </div>
                    <button type="button" class="ms-auto -mx-1.5 -my-1.5 bg-blue-50 text-blue-500 rounded-lg focus:ring-2 focus:ring-blue-400 p-1.5 hover:bg-blue-200 inline-flex items-center justify-center h-8 w-8" data-dismiss-target="#ticket-notification" aria-label="Close">
